...
import type { TeardownConfiguration, TeardownStep } from "../types";
...
function EmptyStep(): TeardownStep {
  return { name: "", step_type: "command", command: "", timeout: 300, on_failure: "continue" };
}

export function TeardownPanel({ selectedConfigId, onSelectConfig }: TeardownPanelProps) {
  const [configs, setConfigs] = useState<TeardownConfiguration[]>([]);
  const [predefinedScripts, setPredefinedScripts] = useState<TeardownScript[]>([]);
  const [showCreate, setShowCreate] = useState(false);
  const [editId, setEditId] = useState<number | null>(null);
  const [name, setName] = useState("");
  const [description, setDescription] = useState("");
  const [steps, setSteps] = useState<TeardownStep[]>([EmptyStep()]);

  const loadConfigs = useCallback(async () => {
    try {
      const resp = await api.get<TeardownConfiguration[]>("/teardown-configurations");
      setConfigs(resp.data);
    } catch (e) {
      console.error("Failed to load teardown configurations", e);
    }
  }, []);

  const loadScripts = useCallback(async () => {
    try {
      const resp = await api.get<TeardownScript[]>("/teardown-scripts");
  ...
  const useScriptAsTemplate = (script: TeardownScript) => {
    setShowCreate(true);
    setEditId(null);
    ...
    setSteps([{
      ...
      step_type: "script",
      command: `python teardown_scripts/${script.filename}`,
      timeout: 300,
      on_failure: "continue",
    }]);
  };

  const updateStep = (idx: number, partial: Partial<TeardownStep>) => {
  ...
  const handleSave = async () => {
    ...
    try {
      if (editId) {
        await api.put(`/teardown-configurations/${editId}`, payload);
      } else {
        await api.post("/teardown-configurations", payload);
      }
      resetForm();
      await loadConfigs();
    } catch (e) {
      console.error("Failed to save teardown configuration", e);
    }
  };

  const handleEdit = (config: TeardownConfiguration) => {
    setEditId(config.id);
    setName(config.name);
  ...
  const handleDelete = async (id: number) => {
    try {
      await api.delete(`/teardown-configurations/${id}`);
      if (selectedConfigId === id) onSelectConfig(null);
      await loadConfigs();
    } catch (e) {
      console.error("Failed to delete teardown configuration", e);
    }
  };
  ...
  return (
    <div className="dvp-test-explorer">
      ...
      <div className="dvp-setup-info">
        Configure post-test teardown steps for cleanup and environment reset.
        Selected configuration runs after each test execution completes.
      </div>
        ...
        {configs.length === 0 && !showCreate ? (
          <div className="dvp-empty-state">
            ...
            <div className="dvp-empty-state__sub">
              Create teardown configurations for cleanup and environment reset after test runs
            </div>
          </div>
        ) : (
          configs.map((config) => (
      ...
      {selectedConfigId && (
        <div className="dvp-suite-selection-summary">
          ✅ Teardown configuration active — will run after tests
        </div>
      )}
    </div>
  );
